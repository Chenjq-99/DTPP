...
def calc_loss(neighbors, ego, ego_regularization, 
              scores, weights, ego_gt, neighbors_gt, 
              neighbors_valid, is_first_stage=True,
              is_scorer_freezing=False):
    """
    neighbors: [B, N, M, T, 3]
    ego(sampled_trajectory): [B, N, T, 7]
    ego_regularization(nn predict): [B, T, 3]
    scores: [B, N]
    weights: [B, 8]
    """
    ...
    if is_scorer_freezing:
        loss = 10 * cmp_loss + irl_loss + 0.1 * regularization_loss
    else:
        if is_first_stage:
            loss = cmp_loss + 5 * irl_loss + 0.1 * regularization_loss + 0.01 * weights_regularization
        else:
            loss = cmp_loss + 10 * irl_loss + 0.1 * regularization_loss + 0.01 * weights_regularization

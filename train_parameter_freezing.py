...
def train_epoch(data_loader, encoder, decoder, optimizer):
    ...
    with tqdm(data_loader, desc="Training", unit="batch") as data_epoch:
        for batch in data_epoch:
            ...
            loss = calc_loss(neighbors=neighbors_trajectories, 
                             ego=first_stage_trajectory, 
                             ego_regularization=ego, 
                             scores=scores, 
                             weights=weights, 
                             ego_gt=ego_gt_future, 
                             neighbors_gt=neighbors_gt_future, 
                             neighbors_valid=neighbors_future_valid, 
                             is_first_stage=True,
                             is_scorer_freezing=True)
            ...
            loss += 0.2 * calc_loss(neighbors=neighbors_trajectories, 
                                    ego=second_stage_trajectory, 
                                    ego_regularization=ego, 
                                    scores=scores, 
                                    weights=weights, 
                                    ego_gt=ego_gt_future, 
...
def model_training(args):
    ...
    # set up model
    model = model = torch.load(args.model_path, map_location=args.device)
    ...
    logging.info("Encoder Params: {}".format(sum(p.numel() for p in encoder.parameters() if p.requires_grad)))
    
    decoder = Decoder(neighbors=args.num_neighbors, max_branch=args.num_candidates, \
                      variable_cost=args.variable_weights)
    ...
    logging.info("Decoder Params: {}".format(sum(p.numel() for p in decoder.parameters() if p.requires_grad)))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_path', type=str, help='path to pre-trained model', default='base_model.pth')

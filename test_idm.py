...
from common_utils import get_filter_parameters, get_scenario_map, get_filter_parameters_for_following_lane_with_or_without_lead 
from new_idm_planner import NewIDMPlanner
...
def build_simulation(experiment, scenarios, output_dir, simulation_dir, metric_dir):
    runner_reports = []
    ...
    for scenario in tqdm(scenarios, desc='Running simulation'):
        ...
        # Planner
        planner = NewIDMPlanner(target_velocity=10.0, 
                                min_gap_to_lead_agent=1.0, 
                                headway_time=1.5, 
                                accel_max=1.0, 
                                decel_max=3.0, 
                                planned_trajectory_samples=40, 
...
def main(args):
    ...
    scenario_filter = ScenarioFilter(*get_filter_parameters_for_following_lane_with_or_without_lead(num_scenarios_per_type=200)) 
    worker = SingleMachineParallelExecutor(use_process_pool=False)
    scenarios = builder.get_scenarios(scenario_filter, worker)

    # begin testing
    print('Running simulations...')
    build_simulation(experiment_name, scenarios, output_dir, simulation_dir, metric_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--scenarios_per_type', type=int, default=1000)
